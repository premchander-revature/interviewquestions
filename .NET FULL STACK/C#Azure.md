...
- Once the development and testing will over for a particular application, we can use Azure for hosting the application.
...
7.  What will happen when SQL Azure database will reach the max size?
...
In SQL Azure, SSL connections are supported only.
...
11. How many of the databases can you create in the single server?
...
12. Can you explain about the SQL Azure security?
...
SQL Azure services will allow you to block the request that will be based on IP address by using the SQL Azure firewall. It will use the SQL server authentication process that will authenticate the connections. By default, SQL Azure connections are SSL encrypted.
...
13. What will be the process to migrate to SQL Azure from SQL server?
...
If we want to migrate from SQL server to SQL Azure, we can use SSIS or BCP. For the schema migration, generate script wizard will be used and we can also use the tool named as SQL Azure Migration Wizard for it.
...
14. How will you sync SQL Azure with on-premise SQL server?
...
It is possible to use No code solution named as DATA SYNC to sync SQL Azure with on-premise SQL server. It is also possible to develop the custom solutions by using SYNC Framework for it.
...
18. How is SQL Azure different than SQL server?
...
All tables must have clustered index. We can't have a table without clustered index.
