...
<img alt="Alt text" src="www.imgur.com/a/jZvtZr2">
...
Problems:
	Following problems may occur due to the impedance mismatch:
        ...
        3.	Impedance mismatch is less of a problem when a special database programming language is designed that uses the same 	data model and data type as a database model for example Oracles’sPL/SQL.
...
- a)	Map the hibernate types explicitly in the mapping file
- b)	Let the hibernate identifies the type automatically
- c)    Map the hibernate types explicitly in the mapping file
