...
1. The BCNF is not utilised when a relation has
...
2. Which of the following normal forms is founded on the idea of "complete functional dependency"?
...
3. In a relational database architecture, take into account the BOOK table and the related dependencies.. What is the normal form for the given relation.
...
4. For the database relation R(a, b, c, d) where the domains of a, b, c and d include only atomic values, and only the following functional dependencies and those that can be inferred from them hold :
...
<b> .</b> a→c (Prime derives Non-Prime.)

<b> .</b> b→d (Prime derives Non-Prime.)

<b> .</b> Prime attribute { a,b}

<b> .</b> Non prime attribute {c,d}.
...
5.  Many-to-one relationship exists between sets r1 and r2. How can it be represented, using functional dependencies if Pk(r) denotes the primary key attribute of relation 'r'?
...
6. Which of the following Normal Forms depends on transitive dependency :
...
- For 3NF every non‐prime attribute of table must be dependent on primary key.
...
7. The primary key is choosen from the:
...
<sumamry><b>Explanation</b></summary>
...
8. Which one of the following statements is not TRUE about Normal Forms?
...
9. Analysis of relational schemas to reduce insertion or update anomalies and redundancy is categorised as:

a. Normalized of data
...
10. Which of the following relation schemas is definitely in Boyce-Codd Normal Form ?
...
11. For the database relation R(a,b,c,d) where the domains of a, b, c and d include only atomic values,, and only the following functional dependencies and those that can be inferred from them hold :
...
a. 1 normal form but not in 2 normal form

b. 2 normal form but not in 3 normal form
...
12. If a table is in Boyce-Codd Normal Form and has no multivalued dependencies, then it is Which Form?
...
13. Which of the following statements is FALSE with respect to normalization?

a. 4NF is more immune against logical inconsistencies than a 3NF table.

b. A 3NF table will have fewer anomalies than a 2NF table

c. A 3NF table is more vulnerable than a 2NF table

d. A database is said to be in 3NF if all its tables are in 3NF
...
14. Which of the following statements is the main reasons for normalizing relation in a database?
...
15. At which level is multi-valued dependency checked among attributes? ?
...
The functional dependencies F = {AB → C, AB → D, C → A, D → B} are best represented by the relation scheme R(A, B, C, Dbest )'s which normal form ?
...
Since single attributes can't determine all the attributes of relation So, now check with two attributes
...
C -> A and D-> B it is not be partial dependency because in C -> A, A is a Key element and in D -> B, B is key element.

So, there is no partial dependency or transitive dependency Therefore it is in 3NF

C and D are not keys for the relation A,B,C,D , therefore it is not in BCNF
...
17. Decomposition aids in removing Which of the issues associated with poor design?
...
1.  Which normal form is usually considered for a normal database design ?
...
- 3NF is considered adequate for normal relational database design because most of the 3NF tables are free of insertion, update, and deletion anomalies.

- However 3NF always ensures functional dependency preserving and lossless and

- BCNF is not considered adequate because BCNF doesn't guarantee dependency preservation.
...
Every relation in Boyce-Codd Normal Form is also in 3rd Normal Form.
...
20. Which of the following statements is true if a relation is in 2NF and 3NF forms:
...
- A relation R is in 2NF if and only if it is in 1NF
- There no partial dependencies
- Partial dependencies means Every non-prime attribute is fully functionally dependent on each relation key
...
21. If each non-key characteristic depends on the primary key to operate, then the relationship is in \_\_\_\_\_\_ .
...
- Given key is Third normal form but Second normal form is the most appropriate one
...
22. The Following relation R (A, B, C, D, E, F, G, H), has a property where each attribute is atomic, and following functional dependencies exist.
...
Then the relation R is \_\_\_\_\_\_?
...
Given R (A, B, C, D, E, F, G, H) with FD's
...
Form the RHS in Given FD's all attributes are present except D So, Attribute D can’t be derived from any key and it must be added to all keys to be derived from.

Hence we have to add attribute D to A,B,C,E,F,G,H and check which of them are Candidate keys of size 2.
...
Candidate keys = AD, BD, ED and FD.

CD+ GD+ HD+ are not candidate keys because it does not derive all the attributes of the given relation R

Partial dependencies = { A → BC, B → CFH and F → EG }

So, the given relation is in 1NF but not in 2NF.
...
23. For a database relation R(A, B, C, D) where the domains of A, B, C and D include only atomic values, , and only the following functional dependencies and those that can be inferred from them hold :

A → C

B → D

The relation R is in \_\_\_\_\_.
